...
| **filters 过滤器**            | ❌ 未使用      | ✅ 使用  | 全局过滤器（如日期格式化）。不使用则无法使用 `{{ date                               |
...
| `src/i18n/`     | ❌ 无          | ✅ 有    | 国际化语言文件。不创建则无法支持多语言                        |
| `src/store/`    | ❌ 无          | ✅ 有    | Pinia/Vuex 状态管理。analytics-stat 使用 Pinia 但未单独建目录 |
| `src/template/` | ❌ 无          | ✅ 有    | 页面模板（如分享页面）。不创建则无法支持独立页面              |
| `src/business/` | ✅ 有          | ✅ 有    | 业务组件                                                      |
| `src/router/`   | ✅ 有          | ✅ 有    | 路由配置                                                      |
| `src/views/`    | ✅ 有          | ❌ 无    | 页面组件（analytics-stat 独有，其他模块放在 business 中）     |

### 6.2 analytics-stat 目录结构
