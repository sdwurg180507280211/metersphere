...
function replaceInSlide(slide, rules) {
    ...
    replaced += replaceInNotesPage(slide, rules);
    return replaced;
}

function replaceInNotesPage(slide, rules) {
    let replaced = 0;
    ...
    try {
        ...
        for (let i = 1; i <= slide.NotesPage.Shapes.Count; i++) {
            replaced += replaceInShape(slide.NotesPage.Shapes.Item(i), rules);
        }
    } catch (e) {
        console.log("备注页替换异常，错误: " + e.message);
    }
...
function replaceFontInSlide(slide, fromFont, toFont) {
    ...
    changed += replaceFontInNotesPage(slide, fromFont, toFont);
    return changed;
}

function replaceFontInNotesPage(slide, fromFont, toFont) {
    let changed = 0;
    ...
    try {
        ...
        for (let i = 1; i <= slide.NotesPage.Shapes.Count; i++) {
            changed += replaceFontInShape(slide.NotesPage.Shapes.Item(i), fromFont, toFont);
        }
    } catch (e) {
        console.log("备注页字体替换异常，错误: " + e.message);
    }
