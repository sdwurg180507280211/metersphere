...
我在做：把脚本里真实的 `docker build` 调用方式点出来。
目的是：你能明确镜像是如何被打出来的（参数、Dockerfile、build context），避免自己手写 `docker build` 时“差一个参数就不一致”。
如果不这样做,就无法实现：你可能会绕开脚本自行构建，导致镜像内容/版本号不一致，从而出现“同样代码，不同机器跑出来不一样”的问题。

- `docker build` 关键点（脚本事实）：
  - `--build-arg MS_VERSION=${IMAGE_VERSION}`：把版本号传进 Dockerfile
  - `-t ${REGISTRY}/${module_name}:${IMAGE_VERSION}`：镜像名与 tag 的拼接规则
  - 简单模块（`gateway` / `eureka`）：
    - Dockerfile 位置：模块目录下的默认 `./Dockerfile`
    - build context：模块目录（`.`）
  - 复杂模块（例如 `test-track` 等）：
    - Dockerfile 位置：`${module_path}/Dockerfile`
    - build context：项目根目录（脚本会 `cd $PROJECT_PATH` 然后以 `.` 作为 context）
...
3. Docker build 阶段（生成镜像）
   ...
   - 版本号注入：`--build-arg MS_VERSION=${IMAGE_VERSION}`
4.（可选）docker save 阶段（导出镜像 tar）
   - 触发条件：`BUILD_ONLY != true`
   - 导出内容：本次脚本实际构建出来的镜像列表（脚本会收集到 `BUILT_IMAGES` 数组，然后 `docker save "${BUILT_IMAGES[@]}" > $PACKAGE_PATH`）
   - 目的：用于离线交付/上机部署（服务器侧通常用 `docker load < xxx.tar` 导入）
