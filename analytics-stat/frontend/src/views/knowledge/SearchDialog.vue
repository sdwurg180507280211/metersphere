<template>
  <n-modal v-model:show="visible" preset="card" :title="t('analytics.knowledge.search_dialog_title')" style="width: 720px" :mask-closable="false" @after-leave="handleClose">
    <div class="search-bar">
      <n-input v-model:value="query" :placeholder="t('analytics.knowledge.search_placeholder')" clearable size="large" @keyup.enter="doSearch">
        <template #suffix>
          <n-button tertiary :loading="loading" @click="doSearch">{{ t('analytics.knowledge.search') }}</n-button>
        </template>
      </n-input>
    ...
    <div v-if="results.length > 0" class="result-list">
      <div v-for="(item, index) in results" :key="`${item.fileMd5}-${item.chunkId}`" class="result-item">
        <div class="result-header">
          <span class="result-index">#{{ index + 1 }}</span>
          <span class="result-filename">{{ item.fileName || item.fileMd5 }}</span>
          <n-tag v-if="item.isPublic" size="small" type="success">{{ t('analytics.knowledge.public') }}</n-tag>
    ...
    <n-empty v-else-if="searched && !loading" :description="t('analytics.knowledge.no_result')" />
  </n-modal>
</template>
