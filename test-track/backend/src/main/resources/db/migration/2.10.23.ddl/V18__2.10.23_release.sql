...
SELECT uuid(),
    ...
    'ISSUE', 0, 2, '1'                     -- scene=ISSUE, required=0（非必填）, order=2（展示顺序）, default_value='1'
WHERE NOT EXISTS (
    SELECT 1
...
-- 策略：
--   1. 如果缺陷没有复测次数记录，插入默认值 1
--   2. 如果缺陷的复测次数为空/NULL/0，更新为 1
--   3. 如果缺陷已有复测次数值（>0），保持不变
...
AND (cfi.value = '' OR cfi.value IS NULL OR cfi.value = '0');

-- 步骤2：为没有"复测次数"字段记录的缺陷插入默认值 1
