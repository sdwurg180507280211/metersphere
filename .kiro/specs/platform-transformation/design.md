...
6. **共享文档可入口化**：V1 支持 Office/WPS/金山/飞书共享文档的上传、链接、打开/预览/嵌入入口；V2 再做深度在线编辑和回写
7. **低侵入**：不重写现有 test-track 核心模块，只改必要入口和集成点
8. **可扩展**：为 V2 在线嵌入编辑、思维图深度编辑、自动通知和全量同步预留字段
...
| 共享文档入口 | 外部链接/iframe/新窗口打开 | V1 支持金山、飞书、WPS、Office 等链接型接入 |
...
### 10. test_workflow_metric_snapshot（指标快照）
