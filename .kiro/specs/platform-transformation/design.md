...
| 外部需求同步 | RocketMQ 4.9.2 | 复用现有 `RequirementSyncConsumer` / `RequirementCallbackProducer` 与全研发双向同步 |
| 平台内部事件 | Kafka | 沿用 MeterSphere 现有内部事件机制，不新增内部 RocketMQ 依赖 |
| 前端框架 | Vue 2.7 + `@micro-zoe/micro-app` | 沿用现有 test-track Vue 2 子应用和主应用微前端架构 |
...
说明：全研发与 MeterSphere 的需求同步沿用当前仓库已有 RocketMQ 适配；MeterSphere 内部服务事件仍沿用 Kafka。前端接入方式按现有 `test-track/frontend` Vue 2 子应用和主应用 `micro-app` 编排实现，不引入额外微前端框架。
...
| actual_start_time | BIGINT | 点击开始准备时记录 |
...
| id | VARCHAR(32) PK | |
| workflow_id | VARCHAR(32) FK | |
| actual_start_time | BIGINT | 实际准备开始时间快照，来源于测试准备阶段 |
...
| case_status_snapshot | LONGTEXT | 用例状态汇总快照 JSON |
| is_locked | TINYINT(1) | 是否锁定 |
| deviation_remark | TEXT | 偏差备注 |
| create_time | BIGINT | |
| update_time | BIGINT | |

### 6. test_workflow_case（流程用例快照）

V1 必须把用于评审和执行的用例结构化落库。Office/WPS 文件、思维图文件和外部链接只作为材料引用；执行阶段以 `test_workflow_case` 为主数据来源。

| 字段 | 类型 | 说明 |
|------|------|------|
| id | VARCHAR(32) PK | |
| workflow_id | VARCHAR(32) FK | |
| source_type | VARCHAR(32) | MANUAL / EXCEL / EXISTING_CASE / EXECUTION_ADDED |
| source_case_id | VARCHAR(50) | 可选关联现有 test_case |
| requirement_num | VARCHAR(64) | 需求编号 |
| split_category | VARCHAR(255) | 需求拆分分类 |
| manual_category | VARCHAR(255) | 人工用例分类，可为空 |
| case_name | VARCHAR(255) | 用例 |
| steps | LONGTEXT | 步骤 |
| expected_result | LONGTEXT | 预期 |
| writer_id | VARCHAR(64) | 用例制定人员 |
| review_status | VARCHAR(32) | PENDING / PASSED / ABOLISHED / INVALID |
| review_remark | TEXT | 评审备注 |
| is_locked | TINYINT(1) | 评审通过后锁定 |
| sort | INT | 排序 |
| create_time | BIGINT | |
| update_time | BIGINT | |

### 7. test_workflow_execution（执行记录）
...
### 9. test_workflow_system_node（系统名称树）
...
### 10. test_workflow_status_log（状态审计）
...
| GET | `/requirement-flow/preparation/{workflowId}/cases` | 获取流程用例快照 |
| POST | `/requirement-flow/preparation/case/save` | 手工保存流程用例快照 |
| POST | `/requirement-flow/preparation/case/import` | Excel 导入流程用例快照 |
| POST | `/requirement-flow/preparation/case/delete` | 删除未锁定的流程用例快照 |
...
| GET | `/requirement-flow/review/{workflowId}/cases` | 获取待评审流程用例快照 |
...
全研发需求同步复用需求池 `RequirementSyncConsumer` / `RequirementCallbackProducer` 的 RocketMQ 实现模式。MeterSphere 内部项目、计划、报告等事件仍沿用现有 Kafka 配置。
...
4. 测试执行完成：同步实际执行起止时间、实际用例数、缺陷数、报告链接
...
3. 页面展示文件链接和下载/预览入口
4. 手工录入或 Excel 导入结构化用例，保存到 `test_workflow_case`
5. 为 V2 预留 `case_file_id`、`case_file_url`、`mindmap_file_id` 等字段
...
V1 生成独立流程报告 `test_workflow_report`，不强行复用现有 `test_plan_report` 表。原因是新报告统计口径来自 `test_workflow_case` 和 `test_workflow_execution`，与现有测试计划报告结构不同。
...
权限落地建议新增项目级资源 `PROJECT_REQUIREMENT_FLOW`，并在 `permission.json`、权限国际化、菜单配置和 DML 初始化脚本中补齐 READ、PLAN_SAVE、PLAN_SUBMIT、PREPARATION_SAVE、REVIEW_SUBMIT、EXECUTE、REPORT_SEND、SYSTEM_NODE_MANAGE、ROLLBACK 等权限点。V1 不建议继续借用 `PROJECT_TRACK_PLAN`，否则无法满足测试组长和测试工程师的职责隔离。
...
| `TestCaseService` | 可选关联现有功能用例；V1 主流程用例来源为 `test_workflow_case` |
| `@SendNotice` | 邮件通知 |
...
| `test-track/backend/src/main/resources/db/migration/2.10.27/ddl/V21__create_test_workflow_tables.sql` | 建表脚本，避免与现有 V20 DML 版本冲突 |
...
| 菜单配置相关文件 | 添加需求测试流程菜单 |
| `test-track/backend/src/main/resources/permission.json` | 添加流程模块权限点 |
| 权限 DML / 国际化相关文件 | 初始化测试组长、测试工程师可用权限和显示文案 |
...
| 用例只存附件导致无法执行 | 高 | V1 必须落 `test_workflow_case` 结构化快照，附件仅作为材料 |
| 技术栈口径与仓库不一致 | 中 | 外部全研发同步沿用 RocketMQ，内部事件沿用 Kafka，前端沿用 micro-app |
