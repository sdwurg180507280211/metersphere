...
- [ ] 1. 主应用 micro-app 基础设施搭建
  - [ ] 1.1 安装 micro-app 依赖并创建初始化配置
    ...
    - 创建 `framework/sdk-parent/frontend/src/micro-app-setup.js`，调用 `microApp.start()` 并配置沙箱和生命周期
  ...
  - [ ] 1.2 创建模块配置表和双模式加载逻辑
    ...
    - 初始状态所有模块标记为 `migrated: false`
    - 修改 `framework/sdk-parent/frontend/src/micro-app.js`，在 `registerMicroApps` 前过滤已迁移模块
  ...
  - [ ]* 1.4 编写属性测试：服务列表到 micro-app 标签映射
    ...
    - 使用 fast-check 生成随机服务列表，验证标签生成逻辑
...
- [ ] 2. 跨应用通信机制改造
  - [ ] 2.1 创建 EventBus 兼容适配器
    ...
    - 实现 `createEventBusAdapter()` 函数，桥接 micro-app dispatch 和本地 Vue EventBus
    - 支持主应用 → 子应用（data 属性）和子应用 → 主应用（dispatch）双向通信
    - _Requirements: 4.1, 4.2_

  - [ ] 2.2 实现事件广播机制
    - 在主应用中实现 `broadcastToAllApps(eventData)` 函数
    - 使用 `microApp.setData()` 向所有已加载的 micro-app 子应用广播事件
...
- [ ] 5. 子应用改造模板和工具
  ...
  - [ ] 5.2 创建子应用 main.js 改造模板
    - 移除 qiankun 生命周期导出（bootstrap、mount、unmount、update）
    - 添加 `window.addEventListener('unmount', ...)` 监听
    - 添加 `window.microApp?.addDataListener(...)` 监听路由更新
    - 将 `!window.__POWERED_BY_QIANKUN__` 替换为 `!window.__MICRO_APP_ENVIRONMENT__`
    - 集成 EventBus 兼容适配器
    - _Requirements: 2.1, 2.3, 2.5_

  - [ ] 5.3 创建子应用 vue.config.js 改造模板
    - 移除 `library` 和 `libraryTarget: 'umd'` 配置
    - 保留 `chunkLoadingGlobal` 避免多应用冲突
    - 保留 CORS 头配置
...
- [ ] 6. 第一批子应用迁移（简单模块）
  - [ ] 6.1 迁移 workstation 子应用
    ...
    - 按模板改造 `workstation/frontend/src/main.js`
    - 按模板改造 `workstation/frontend/vue.config.js`
    - 在模块配置表中将 workstation 标记为 `migrated: true`
    - _Requirements: 2.1, 2.2, 2.3, 2.4, 2.5, 2.6_

  - [ ] 6.2 迁移 report-stat 子应用
    - 按模板改造 `report-stat/frontend/src/public-path.js`
    - 按模板改造 `report-stat/frontend/src/main.js`
    - 按模板改造 `report-stat/frontend/vue.config.js`
    - 在模块配置表中将 report-stat 标记为 `migrated: true`
    - _Requirements: 2.1, 2.2, 2.3, 2.4, 2.5, 2.6_

  - [ ] 6.3 迁移 analytics-stat 子应用
    - 按模板改造 `analytics-stat/frontend/src/public-path.js`
    - 按模板改造 `analytics-stat/frontend/src/main.js`
    - 按模板改造 `analytics-stat/frontend/vue.config.js`
    - 在模块配置表中将 analytics-stat 标记为 `migrated: true`
    - _Requirements: 2.1, 2.2, 2.3, 2.4, 2.5, 2.6_
...
- [ ] 8. 第二批子应用迁移（中等复杂度）
  - [ ] 8.1 迁移 project-management 子应用
    - 按模板改造 public-path.js、main.js、vue.config.js
    - 在模块配置表中标记为 `migrated: true`
    - _Requirements: 2.1, 2.2, 2.3, 2.4, 2.5, 2.6_
...
- [ ] 9. 第三批子应用迁移（高复杂度 - 含按需加载）
  ...
  - [ ] 9.2 迁移 api-test 子应用
    - 按模板改造 public-path.js、main.js、vue.config.js
    - 注意 api-test 有多个 pages 入口（shareApiReport、shareDocument、apiDocument），需确保非主入口不受影响
    - 注意 api-test 使用 `microRouter`（内存路由）处理按需加载场景
    - 在模块配置表中标记为 `migrated: true`
    - _Requirements: 2.1, 2.2, 2.3, 2.4, 2.5, 2.6_
